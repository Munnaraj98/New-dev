...
def handle_attack(message):
    ...
    if message.chat.id != int(GROUP_ID):
        bot.reply_to(message, "This bot can only be used in the specified group. Join - https://t.me/+PbJPDGt1VFhkMzVl")
        return

    # Ensure user is a member of the channel
    if not is_user_in_channel(user_id):
        bot.reply_to(message, f"You must join {CHANNEL_USERNAME} to use this bot. @rajaraj_04")
        return
